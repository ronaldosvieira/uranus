	...
	public boolean getInstruction() throws AccessErrorException, IntegerOutofRangeException, OverflowException {
		...
		if(m.getWord(pc)!=null){
			...
			if(instrucao.startsWith("000000")){
				...
				switch (function) {
					case "100000":
						rs = p.getRegisterByAddress(instrucao.substring(6, 11));
						rt = p.getRegisterByAddress(instrucao.substring(11, 16));
						rd = p.getRegisterByAddress(instrucao.substring(16, 21));
						ALU.add(rs, rt, rd);
						break;
					case "100001":
						rs = p.getRegisterByAddress(instrucao.substring(6, 11));
					...
					case "011000": {
						rs = p.getRegisterByAddress(instrucao.substring(6, 11));
						rt = p.getRegisterByAddress(instrucao.substring(11, 16));
						Register hi = p.getRegisterByAddress("hi");
						Register lo = p.getRegisterByAddress("lo");
					...
					case "000000":
						...
						break;
					case "001000":
						pc = p.getRegisterByAddress(instrucao.substring(6, 11)).getValue();
						break;
					case "001100":
						rt = p.getRegisterByAddress(instrucao.substring(11, 16));
						rd = p.getRegisterByAddress(instrucao.substring(16, 21));
				...
				imediato = Integer.parseInt(instrucao.substring(6), 2);
				p.getRegisterByAddress("11111").setValue(pc);
				pc = imediato-4;
			}
			pc +=4;
		}
